...
echo "--------------------DR--------------------" > ./dr.txt
echo "" >> ./dr.txt
for i in `ls $datesource*.log`; do cat $i | grep 'Unable to get destination batch ID for TID,MID,acquirer*' >> /home/linux/dr.txt;  done
...
echo "--------------------PR--------------------" > ./pr.txt
echo "" >> ./pr.txt
for i in `ls $datesource*.log`; do cat $i | grep 'Unable to get destination batch ID for TID,MID,acquirer*' >> /home/linux/pr.txt;  done
...
msg="This is the daily report for Unable to get destination batch ID for TID,MID,acquirer cases from PowerZac for yesterday.\n\nThis is an automatically generated email, please do not reply.\n\n\nIoannis Atlamazoglou"
...
echo -e $msg | mutt -s "PZAC Unable to get dest. batch id" -a ./full_totals.txt -- $email1 $email2 $email4 $email17 $email18 
