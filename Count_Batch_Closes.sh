...
grep 'Raw.*.AO60' test | cut -d " " -f 2 | cut -d "]" -f 1 > DATE
grep 'Raw.*.AO60' test | cut -d " " -f 9 | cut -d "." -f 2 | cut -c 3-10 > TID
paste -d " " DATE TID > bc_dr.txt
rm DATE TID test
...
grep 'Raw.*.AO60' test | cut -d " " -f 2 | cut -d "]" -f 1 > DATE
grep 'Raw.*.AO60' test | cut -d " " -f 9 | cut -d "." -f 2 | cut -c 3-10 > TID
paste -d " " DATE TID > bc_pr.txt
rm DATE TID test




msg="This is the daily report for batch closes from PowerZac for yesterday.\n\nThis is an automatically generated email, please do not reply.\n\n\nIoannis Atlamazoglou"
...
echo -e $msg | mutt -s "PZAC batch closes" -a ./bc_dr.txt -a ./bc_pr.txt -- $email1 $email2 $email14 $email15 $email16 $email13 $email10 $email17 $email18


rm bc_dr.txt bc_pr.txt;
rm ./Videos/*;
