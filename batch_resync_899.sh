...
grep 'Raw.*.FO00050899' test | cut -d " " -f 2 | cut -d "]" -f 1 > DATE
grep 'Raw.*.FO00050899' test | cut -d " " -f 9 | cut -d "." -f 2 | cut -c 3-10 > TID

paste -d " " DATE TID >> br_899_dr.txt
rm DATE TID test
...
paste -d " " DATE TID >> br_899_pr.txt
rm DATE TID test
...
msg="This is the daily report for batch resync – 899 response from PowerZac for yesterday.\n\nThis is an automatically generated email, please do not reply.\n\n\nIoannis Atlamazoglou"
...
echo -e $msg | mutt -s "PZAC batch resync – 899 response" -a ./br_899_dr.txt -a ./br_899_pr.txt -- $email1 $email4 $email2 $email17 $email18 



rm br_899_dr.txt br_899_pr.txt;
rm ./Videos/*;
