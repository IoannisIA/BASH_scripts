...
with open("/home/linux/settlement_type_nbg.txt") as fd:
    line = fd.readline()
    ...
    while line:
        ...
        if row_type == '02':
            ...
            pan1 = line[43:62].strip()
            pan = pan1[:6]+"*****"+pan1[-4:]
            exp_date = line[91:95]+'01'
            if exp_date[2:4] == "00":
                exp_date = line[91:93]+"0101"
            trx_datetime = line[10:24]
            trx_amount = int(line[64:74])/100
            rrn = line[164:173]
            auth_code = line[74:80]
            ...
            if (tid+batch_num+file_settl_date+"1") in dict_buffer_batch:
                dict_buffer_batch[tid+batch_num+file_settl_date+"1"]["batch_trx_count"] += 1
                if trx_type == "PURCHASE" :
                    dict_buffer_batch[tid+batch_num+file_settl_date+"1"]["batch_debits"] += 1
                    dict_buffer_batch[tid+batch_num+file_settl_date+"1"]["batch_debits_total"] += trx_amount
                    dict_buffer_batch[tid+batch_num+file_settl_date+"1"]["batch_net_amount"] += trx_amount
                else:
                    dict_buffer_batch[tid+batch_num+file_settl_date+"1"]["batch_credits"] += 1
                    dict_buffer_batch[tid+batch_num+file_settl_date+"1"]["batch_credits_total"] += trx_amount
                    dict_buffer_batch[tid+batch_num+file_settl_date+"1"]["batch_net_amount"] -= trx_amount

            else:

                if trx_type == "PURCHASE":
                    dict_buffer_batch.setdefault(tid+batch_num+file_settl_date+"1",
            ...
            if tid in dict_buffer_terminal:
                ...
                if trx_type == "PURCHASE":
                    dict_buffer_terminal.setdefault(tid,
                        {"key": mid+file_settl_date+"1", "terminal_trx_count": 1, "terminal_mid": mid, "terminal_cur": "978", "terminal_credits": 0, "terminal_credits_total": 0,
                         "terminal_debits": 1, "terminal_debits_total": trx_amount, "terminal_net_amount": trx_amount})
                else:
                    dict_buffer_terminal.setdefault(tid,
                        {"key": mid+file_settl_date+"1", "terminal_trx_count": 1, "terminal_mid": mid, "terminal_cur": "978", "terminal_credits": 1, "terminal_credits_total": trx_amount,
            ...
            if mid in dict_buffer_merchant:
                ...
                if trx_type == "PURCHASE":
                    dict_buffer_merchant.setdefault(mid,
                        {"key": mid+file_settl_date+"1", "mid_trx_count": 1, "mid_name": mid_name, "mid_cur": "978", "mid_credits": 0, "mid_credits_total": 0,
                         "mid_debits": 1, "mid_debits_total": trx_amount, "mid_net_amount": trx_amount})
                else:
                    dict_buffer_merchant.setdefault(mid,
                        {"key": mid+file_settl_date+"1", "mid_trx_count": 1, "mid_name": mid_name, "mid_cur": "978", "mid_credits": 1, "mid_credits_total": trx_amount,
            ...
            try:
                ...
                list_buffer_trxs.append((tid+batch_num+file_settl_date+"1", trx_type, pan, exp_date, trx_datetime, trx_amount, rrn, auth_code, card_type, entry_mode, ident_method, auth_method, install))
